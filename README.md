...
- `generate_signing_key` (one-time) creates the project's GPG keypair under `<dir_gnupg>/signing/`; `verify_signing_key` sign+verifies a test payload to confirm it's usable; `print signing` shows the key's fingerprint + uid + state.  The signed apt-repo (CONF-01/02 phase 2) builds on top of this; for now the key just sits there ready to be used.  See `[Repo] SigningKeyUid` in `config/build.conf` for the identity string.  **`build_chroot` automatically copies the exported public key into the chroot at `/usr/share/keyrings/athena-archive-keyring.gpg`** when a key exists — the conventional distro-signing-key location, ready for any future `apt source [signed-by=...]` entry pointing at the Athena repo.  If you generate the key after first running `build_chroot`, re-run `build_chroot` to pick it up; `verify_chroot` reports keyring presence/absence on every run.
