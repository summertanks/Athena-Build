...
2. In the WSL distro, confirm the engine is reachable:
   ...
   ```
3. On a fresh checkout, commands are gated until you run **`configure`** (see
   "First-run setup" in `docs/mirror-setup.md`): pick **federation peer**, let
   it validate the tier-1 key + register to the mirror, then choose **build**
   mode.  Mode and registration are stored in the untracked `config/local.conf`
   — *not* `build.conf` — so they never leak through git.  Populate
   `config/build_pkg.list` with the packages this peer owns.  (Build mode is
   refused until the peer has registered to a mirror.)
