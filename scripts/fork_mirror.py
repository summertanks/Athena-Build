...
def generate_fork_mirror(buildconfig: 'utils.BuildConfig') -> bool:
    ...
    if getattr(buildconfig, 'audit_identity_scan', True):
        ...
        logger.warning(
            "fork_mirror: identity audit DISABLED via "
            "[Audit] IdentityScan = false — Debian residue can ship "
            "without flagging.  Re-enable for production builds."
        )
...
def _wipe_fork_pkg_outputs(pkg_name: str, binary_names: List[str],
    ...
    Targets:
      ...
      - log/build/<src>{,.build.json} (build log + signed record)
    """
    _targets: List[str] = []
    ...
    for _suffix in ('', utils.BUILD_RECORD_SUFFIX):
