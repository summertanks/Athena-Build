...
def rescan_outputs_from_repo(
    buildlog_dir: str, repo_root: str,
    source_lookup: 'Callable[[str], Optional[Any]]',
) -> dict:
    """For each build.json record, walk ``repo_root`` for every file
    whose basename starts with one of the source's declared binary
    names and add any not already in ``outputs``/``output_hashes``.

    Fixes silent incompleteness in historical records caused by
    `_segregate_built_artifacts`'s 'kept-existing' branch (fixed
    upstream 2026-06-06): when a rebuild produced a byte-identical
    artifact whose destination already existed, the old code dropped
    the rebuilt dup AND didn't append the existing destination path
    to `_moved_paths`.  Downstream `output_hashes` population then
    silently missed those files.  Affected all udebs (497 on disk,
    0 in any build.json) and a long tail of byte-identical rebuilds.

    `source_lookup(name) → Source-like` must return the Source object
    whose ``binary`` attribute lists every binary name the source
    declares.  We then look up each name in the repo index and add
    any matching files.

    Returns ``{scanned, augmented, files_added}``.
    """
    _idx: 'dict[str, set]' = {}
    for _root, _dirs, _files in os.walk(repo_root):
        for _f in _files:
            ...
            if len(_parts) != 3:
                continue
            _idx.setdefault(_parts[0], set()).add(
                os.path.join(_root, _f))

    _stats = {'scanned': 0, 'augmented': 0, 'files_added': 0}
    try:
    ...
    for _entry in _entries:
        ...
        if _rec is None:
            continue
        _src = source_lookup(_pkg)
        if _src is None:
            continue
        _binaries = list(getattr(_src, 'binary', []) or [])
        if not _binaries:
            continue
        _outputs = list(_rec.get('outputs') or [])
        _output_hashes = dict(_rec.get('output_hashes') or {})
        _existing_set = set(_outputs)
        _added_this_record = 0
        for _b in _binaries:
            for _path in sorted(_idx.get(_b, set())):
                _basename = os.path.basename(_path)
                if _basename in _existing_set:
                    continue
                _h = get_sha256(_path)
                if not _h:
                    continue
                _outputs.append(_basename)
                _output_hashes[_basename] = _h
                _existing_set.add(_basename)
                _added_this_record += 1
        if _added_this_record == 0:
            continue
        _rec['outputs'] = sorted(_outputs)
        _rec['output_hashes'] = _output_hashes
        _rec['output_count'] = len(_outputs)
        try:
            write_build_record(buildlog_dir, _rec)
            _stats['augmented'] += 1
            _stats['files_added'] += _added_this_record
        except OSError as _e:
            logger.warning(
                f"rescan_outputs_from_repo {_pkg}: write failed: {_e}")
    return _stats
