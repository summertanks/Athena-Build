...
_OVERLAY_MAP = [
    ...
    # Phase 2 of GROUPS-01: pre-pkgsel hook that copies the per-build
    # tasksel `.desc` from `/cdrom/.disk/athena-tasks.desc` into
    # `/target/usr/share/tasksel/descs/` before pkgsel runs.  The
    # `.desc` is generated by `iso_installer._stage_tasksel_desc`
    # from the operator-defined groups in pkg.list.
    ('pkgsel/pre-pkgsel.d-athena-tasks',
     'usr/lib/pre-pkgsel.d/05-athena-tasks'),
]
