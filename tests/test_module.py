...
# ─── UX-04 commit B: persistence module (pickle Cache + DepTree) ──────────

class _DummyPicklable:
    """A trivial picklable object for persistence-roundtrip tests that
    don't need a real Cache / DependencyTree.  Carries a marker so we
    can distinguish a freshly-constructed instance from a successfully-
    unpickled one."""
    def __init__(self, marker='ORIGINAL'):
        self.marker = marker


def test_persistence_sha256_roundtrip(tmp_path=None):
    """_sha256_of_file matches what we'd expect from sha256sum on the
    same bytes; _verify_sha256 returns True on a matching sidecar."""
    import tempfile
    import hashlib
    from persistence import _sha256_of_file, _write_sha256, _verify_sha256
    with tempfile.TemporaryDirectory() as tmp:
        blob = os.path.join(tmp, 'blob.bin')
        with open(blob, 'wb') as fh:
            fh.write(b'hello world')
        expected = hashlib.sha256(b'hello world').hexdigest()
        assert _sha256_of_file(blob) == expected
        _write_sha256(blob)
        assert _verify_sha256(blob) is True


def test_persistence_sha256_mismatch_refuses():
    """_verify_sha256 returns False when sidecar's hex doesn't match
    the file's actual digest — corruption / tamper detection."""
    import tempfile
    from persistence import _verify_sha256, _write_sha256
    with tempfile.TemporaryDirectory() as tmp:
        blob = os.path.join(tmp, 'blob.bin')
        with open(blob, 'wb') as fh:
            fh.write(b'original')
        _write_sha256(blob)
        # Mutate the blob without updating the sidecar
        with open(blob, 'wb') as fh:
            fh.write(b'tampered')
        assert _verify_sha256(blob) is False


def test_persistence_sha256_missing_sidecar_refuses():
    """Without a .sha256 file, _verify_sha256 returns False (rather
    than trusting the blob unconditionally)."""
    import tempfile
    from persistence import _verify_sha256
    with tempfile.TemporaryDirectory() as tmp:
        blob = os.path.join(tmp, 'blob.bin')
        with open(blob, 'wb') as fh:
            fh.write(b'data')
...
def test_persistence_save_load_roundtrip_via_dummy_object():
    """_save_pickled + _load_pickled round-trip a picklable object
    through the gzip + SHA256 envelope.  Uses _DummyPicklable rather
    than a real Cache to keep this fast and isolate the format-version
    handling from the real classes' pickling shape."""
    import tempfile
    from persistence import _save_pickled, _load_pickled
    with tempfile.TemporaryDirectory() as tmp:
        path = os.path.join(tmp, 'x.pkl.gz')
        original = _DummyPicklable(marker='OK')
        assert _save_pickled(original, path, format_version=1) is True
        assert os.path.exists(path)
        assert os.path.exists(path + '.sha256')
        loaded = _load_pickled(path, expected_version=1)
        assert loaded is not None
        assert loaded.marker == 'OK'


def test_persistence_load_pickled_format_version_mismatch_refuses():
    """Saved at version 1, asked to load as version 2 — refuse with
    None + WARNING (don't silently load)."""
    import tempfile
    from persistence import _save_pickled, _load_pickled
    with tempfile.TemporaryDirectory() as tmp:
        path = os.path.join(tmp, 'x.pkl.gz')
        _save_pickled(_DummyPicklable(), path, format_version=1)
        result = _load_pickled(path, expected_version=2)
        assert result is None


def test_persistence_load_pickled_missing_returns_none():
    """No file → None.  No warning needed (this is the "fresh build"
    path, expected on first run)."""
    import tempfile
    from persistence import _load_pickled
    with tempfile.TemporaryDirectory() as tmp:
        path = os.path.join(tmp, 'absent.pkl.gz')
        assert _load_pickled(path, expected_version=1) is None


def test_persistence_load_pickled_corrupt_blob_refuses():
    """Blob is gzip-corrupt → _verify_sha256 either passes (if sidecar
    matches the corrupt bytes) or fails; if it passes, the unpickle
    step catches the corruption.  Either way: None, no exception."""
    import tempfile
    from persistence import _load_pickled, _write_sha256
    with tempfile.TemporaryDirectory() as tmp:
        path = os.path.join(tmp, 'x.pkl.gz')
        with open(path, 'wb') as fh:
            fh.write(b'definitely not gzip')
        _write_sha256(path)  # SHA matches but gzip won't open
        assert _load_pickled(path, expected_version=1) is None


def test_persistence_dep_tree_load_returns_none_when_cache_is_none():
    """load_dep_tree refuses with None when called with cache=None —
    a DT without its back-ref is useless and silent unpickling
    would set up a self.cache=None deref later.  Explicit refusal
    beats deferred crash."""
    import tempfile
    from persistence import load_dep_tree
    with tempfile.TemporaryDirectory() as tmp:
        class _Cfg:
            dir_cache = tmp
        assert load_dep_tree(_Cfg(), cache=None) is None


def test_persistence_dependencytree_pickle_roundtrip_drops_and_rewires_cache():
    """End-to-end: build a small DependencyTree against a stub Cache,
    save_dep_tree, load_dep_tree with a DIFFERENT Cache instance —
    the DT unpickles cleanly and its back-ref is the new cache."""
    import tempfile
    from persistence import save_dep_tree, load_dep_tree
    import dependencytree

    # Two distinct Cache stubs — original (used for save) and
    # replacement (used for load).  We assert the loaded DT's
    # back-ref is the REPLACEMENT, proving __getstate__ dropped
    # the original and load_dep_tree re-wired correctly.
    class _Cache:
        def __init__(self, label): self.label = label
    original_cache = _Cache('original')
    replacement_cache = _Cache('replacement')

    dt = dependencytree.DependencyTree.__new__(dependencytree.DependencyTree)
    # Set just enough attrs that pickle has something to write.
    dt._DependencyTree__cache = original_cache
    ...
    with tempfile.TemporaryDirectory() as tmp:
        class _Cfg:
            dir_cache = tmp
        assert save_dep_tree(dt, _Cfg()) is True
        loaded = load_dep_tree(_Cfg(), cache=replacement_cache)
        assert loaded is not None
        assert loaded._DependencyTree__cache is replacement_cache
        assert loaded._DependencyTree__cache is not original_cache
        assert loaded.arch == 'amd64'
...
def main() -> int:
    tests = [
        ...
        # UX-04 commit B: persistence module (Cache + DepTree pickle)
        test_persistence_sha256_roundtrip,
        test_persistence_sha256_mismatch_refuses,
        test_persistence_sha256_missing_sidecar_refuses,
        test_persistence_save_load_roundtrip_via_dummy_object,
        test_persistence_load_pickled_format_version_mismatch_refuses,
        test_persistence_load_pickled_missing_returns_none,
        test_persistence_load_pickled_corrupt_blob_refuses,
        test_persistence_dep_tree_load_returns_none_when_cache_is_none,
        test_persistence_dependencytree_pickle_roundtrip_drops_and_rewires_cache,
        # UX-04 commit A: BuildFlags JSON persistence
