...
def test_v2_dispatcher_tab_switch_by_index():
    ...
    threading.Thread(target=d.run, daemon=True).start()
    time.sleep(0.02)
    d.post(KeyEvent('KEY_F(3)'))   # cache (index 2 — console, log, cache)
    time.sleep(0.05)
...
def test_per_module_logger_names_pin_routing():
    ...
    'athena' routes to the 'log' catch-all (cross-stage helpers plus
    the tui.py command-handler error path)."""
    import re
    expected = {
        ...
        'utils':            'athena.cache',
        # cross-stage / generic — bare 'athena' → log tab catch-all
        'signing':          'athena',
        'apt_repo':         'athena',
        'repo_audit':       'athena',
        'persistence':      'athena',
