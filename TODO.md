...
| ARCH-12 | P1 | todo | Replace `BuildSystem.build_chroot`'s try-fail-retry round loop with a single-pass topo-sorted install. Today the loop computes Pre-Depends order for unpack only and lets `dpkg --configure -a` sort out Depends ordering on its own — when dpkg's single-pass solver hits a transient unmet dep it bails, and we retry the whole round. Fix: build one graph over `Pre-Depends ∪ Depends`, Kahn-sort it (the unused `BuildSystem.get_install_sequence` already does most of this), break known cycles explicitly (libc seed pattern, perl-base/dpkg pair), then issue named `dpkg --configure pkg1 pkg2 …` instead of `--configure -a`.  Eliminates the round loop, makes errors actionable, and removes the partial-failure noise that `43310fd` papered over. |
