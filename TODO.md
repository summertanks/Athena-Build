...
| CONS-11 | P2 | todo | **scp recovery not reconciled with the build marker** (remote_orchestrate.py:~313). `run_remote` pulls `out/*.deb` back with `stderr=DEVNULL` and no returncode check, then returns the marker's output list — a partial scp yields a `phase=done` record with fewer .debs than the remote built. Fix: check scp rc + assert recovered-file count == marker outputs, transport-retry on mismatch. |
