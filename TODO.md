...
| ID    | Sev | Status | Title |
|-------|-----|--------|-------|
| STA-19 | P1 | todo | **`repo_audit._read_signed_manifest` — fail closed on signing-setup failure.**  Today (`scripts/repo_audit.py:570`) catches a broad `Exception` from `import signing` / homedir lookup, logs a WARNING, and falls through to read the file UNVERIFIED.  Same shape on the write side (`_write_signed_manifest:544`) — signing failure logs WARNING but still writes an unsigned manifest, and the caller can't distinguish "manifest written + signed" from "manifest written, signature failed".  The local signed manifest is the authority for `+asg<R>u<N>` bump-N derivation (consumed by `published_ledger` → `highest_asg_update`).  Quiet trust degradation here lets a publish silently mint colliding `+asg uN` numbers when signing breaks.  **Fix:** raise (or return distinct sentinel) instead of fall-through; surface the failure to the operator at `repo publish` time.  Identified by the 2026-05-28 consolidation audit (Candidate 9). |
| STA-20 | P2 | todo | **Consolidate `_scan_packages_*` shell helpers.**  Two near-parallel f-string-into-`sudo bash -c "cd {cwd} && {argv} > {tmp}"` patterns: `apt_repo._scan_packages_to` (apt_repo.py:653) and `repo_audit._scan_packages_with_progress` (repo_audit.py:167).  Different tempfile/ownership/error/allow-empty contracts mean a refactor needs a unifying interface covering both.  Also `apt_repo._scan_sources_to`, `_write_subdir_release` follow the same shell-string-with-f-string-interpolation pattern.  Callers are all internal (paths from `config.deb_dir_for`) so injection risk is low, but the duplication is real and the cwd-via-`bash -c "cd"` choreography is fragile.  Identified by the 2026-05-28 consolidation audit (Candidate 7).  Defer until two-or-more change pressure exists on either side. |
