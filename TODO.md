...
> FIXED in the same pass (triad-gated); CONS-10..16 are deferred (each needs a
> design decision or a larger change).  Close deferred rows here as they land.
...
| CONS-10 | P1 | todo | **`source remotebuild` under a pending update silently ships unstamped (non-+asg) artifacts.** `_resolve_build_workload(remote=True)` skips update-mode and `_in_update_build` is never set, so `+asg` stamping never fires; those binaries collide with prior-generation pristine names and get dropped as dups with no runtime warning. Fix: WARN/refuse remotebuild when an update delta is pending (or thread update-mode through the remote path). (cmd_source.py.) |
