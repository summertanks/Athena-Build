...
| ARCH-07 | P2 | done | Decide on a logging story: today output is split between `print()` for stdout, `tui.console.print()` for the curses console tab, `console.error/warning/info` for the log tab, and ad-hoc files (`chroot-install.log`, `mksquashfs.log`, etc.). Wrap with a single `logging` adapter that routes by level. *(Stdlib `logging` adapter shipped in `tui.py`: `logging.getLogger('athena')` is bound by `Tui.__init__` to two handlers — `_LogTabHandler` (INFO/WARNING/ERROR → log tab) and `_ConsoleTabHandler` (custom `DISPLAY=25` level → console tab).  Existing `tui.console.X` call sites unchanged; both APIs converge at the same `Tui.{print,INFO,WARNING,ERROR}` sinks, so `console.X` and `logger.X` are interchangeable for new code.  Per-command capture files (chroot-install.log, mksquashfs.log) remain raw subprocess capture — outside the Python-records story; future ticket.  7 new tests cover the four routing paths, the `attribute` extra, DEBUG suppression, and idempotent re-setup.  53/53 tests pass.)* |
