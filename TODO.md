...
| CONF-11 | P2 | todo | **Fork-tree internal completeness audit at cache-build.**  Surface latent breakages where a `fork/source/<pkg>/` tree's **own build system references a file that isn't there**.  **Why:** discovered 2026-05-18 — `athena-tasksel`'s `Makefile install` target ran `install packages/list …` but `packages/list` was missing from the fork (never tracked in git, probably deleted in an unrecorded cleanup pass shortly after the initial `apt-get source tasksel` seed); the original successful build's `athena-tasksel*.deb` survived in `repo/` and `check_build` skipped subsequent rebuilds, hiding the problem until step-7 prep deleted the existing artifact to force a fresh build.  Symptom: `install: cannot stat 'packages/list': No such file or directory` deep inside the Makefile install target.  Generalised: ANY fork pkg could have a similar latent gap — there's no automated check.  **Design reframed 2026-05-31** away from the original "diff fork against upstream tarball + per-pkg `.upstream-omit` allow-list" shape because forks legitimately trim hundreds of files (locales we don't carry, task descriptions we replace, examples we drop) and the allow-list maintenance cost wiped out the value.  **Reframed mechanism — fork-internal:** at fork-mirror generation time, for each `fork/source/<pkg>/`: (a) parse `debian/<binary>.install` (one path per line — the dpkg packaging manifest) and verify each referenced file exists in the fork tree; (b) grep `Makefile` and `debian/rules` for `install -m … <relative-path>` / `cp <src> <dst>` patterns whose path doesn't start with `$` (i.e. concrete paths, not variable-expanded), check those exist too.  Emit a warning per dangling reference; optional escalation: fail cache build (rather than just warn) when the count exceeds a threshold.  No upstream tarball fetch, no per-pkg allow-list, no triage burden — the audit is surgical and tied directly to the failure mode.  **The athena-tasksel incident would have been caught:** its Makefile references `packages/list` which doesn't exist in the fork.  **Trade-off acknowledged:** misses files that upstream had + we accidentally lost + our own build system happens to NOT reference (no `install`/`cp` line names them).  Narrower coverage, but the catch rate on actual failures is the same.  **Effort:** S–M (1–2 days), contained to `scripts/fork_mirror.py`, ~6–8 unit tests with synthetic fork trees + planted gaps.  Memory `feedback_verify_patch_layer_via_logs.md` is the related principle (don't trust a build-system success without inspecting what it actually did).  Memory `feedback_dh_helper_files_use_binary_name.md` is the cousin discipline (the binary-name dh-helper rule already catches a different fork-tree shape problem). |
...
| AUDIT-01 | P1 | todo | **Identity-leakage scanner — generalize CONF-10 candidate (a) into a CI-gated audit.**  Today's identity strip is per-incident (`installer_chroot._strip_debian_residue_hooks` carries a hardcoded path list; one `test_strip_debian_residue_from_override_lines` test).  Adds `scripts/identity_scan.py` (or `cmd_audit_identity` in `scripts/build.py`) that walks every shipped artifact — chroot tree, installer udeb contents, `installer/finish-install/*`, `installer/grub.cfg`, `/etc/os-release`, `/etc/motd`, `/etc/issue`, `pre-pkgsel.d/*`, baked-in `apt-install X` calls — and greps for `Debian`, `debian.org`, `Bug:`, `report-bug`, `reportbug`, `popularity-contest`, `apt-listchanges`, `bug-buddy`, `installation-report`.  Allowlist file `audit/identity-allowlist` (path + token + reason).  Wire into the test suite as `test_no_unallowlisted_identity_leakage` (registered via `tests/test_module.py:main()`) so it runs in the existing CI ruff/mypy/tests job.  Closes the "list rots on upstream rebuilds" failure mode in CONF-10.  Cross-ref: CONF-10 stays open as the broader residue-audit / hook-handling redesign; AUDIT-01 ships the durable mechanism CONF-10 candidate (a) names.  Filed 2026-05-29 from the comparative-analysis capability-gap audit. |
